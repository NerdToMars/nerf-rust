    ...
    **kwargs,
    ...
    if ndc:
        ...
        if is_debug:
            ...
            rays = []
            ray_length = 1.0
            for i, (ray_o, ray_d) in enumerate(
                zip(rays_o.cpu().numpy(), rays_d.cpu().numpy())
            ):
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    dists = torch.cat(
        [dists, torch.Tensor([1e10]).expand(dists[..., :1].shape).to("mps")], -1
    )  # [N_rays, N_samples]
    ...
    for i in trange(start, N_iters):
        ...
        if use_batching:
            ...
            transform3d = rr.Transform3D(
                translation=cpu_pose[:3, -1], mat3x3=cpu_pose[:3, :3]
            )  # 3x4 RT
            ...
            rr.log(
                f"{camera_entity_name}/image",
                rr.Pinhole(image_from_camera=K, camera_xyz=rr.ViewCoordinates.RUB),
            )
            ...
            if N_rand is not None:
                ...
                if is_debug:
                    ...
                    for i, (ray_o, ray_d) in enumerate(
                        ...
                        rerun_arrow = rr.Arrows3D(
                            origins=ray_o, vectors=ray_d * ray_length
                        )
                        rays.append(rerun_arrow)
                        rr.log(f"world/rays_o_{img_i}/arrow_{i}", rerun_arrow)
